        ...
        if len(constraints):
            ...
            try:
                if len(bits):
                    ...
                    if all(k in constraints for k in ('source', 'destination', 'dateofdeparture')):
                        if constraints['dateofarrival']:
                            dateofarrival = constraints['dateofarrival']
                        else:
                            dateofarrival = None
                        if constraints['seatingclass']:
                            seatingclass = constraints['seatingclass']
                        else:
                            seatingclass = "E"
                        if constraints['adults']:
                        ...
                        if constraints['children']:
                            ...
                            children = 0
                        if constraints['infants']:
                            infants = constraints['infants']
                        else:
                            infants = 0
                        if constraints['counter']:
                            counter = constraints['counter']
                        else:
                            counter = 100

                        GO = goibiboAPI("653332e6", "7c2191040b1d951516221290d893741a")

                        # If any of the above values are present, Call API using values for optional parameters
                        # -- else, call API with default values.                        

                        print GO.FlightSearch("CCJ", "HYD", 20170910, dateofarrival, seatingclass, adults, children, infants, counter)
                        #return GO.FlightSearch(source,destination,dateofdeparture,dateofarrival,seatingclass,adults,children,infants,counter)

                        ## TO DO...THE returned data not in the required format. Change accordingly.
